...
def preprocess_data_spark():
    data = spark.read.csv(f"{OUTPUT_DATA_DIR}/telco_customer_churn.csv", header=True, inferSchema=True)
    ...
    data.toPandas().to_csv(f"{OUTPUT_DATA_DIR}/preprocessed_telco_customer_churn.csv", index=False)
